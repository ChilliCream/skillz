...
public class XdgPathsTests
{
    ...
    [Fact]
    public void GetLegacyGlobalLockPaths_FallsBackToDotAgents_When_NoEnv()
    {
        // Arrange
        var paths = Create();
        ...
        // Act & Assert
        Assert.Equal(
            new[] { Path.Combine(Home, ".agents", ".skill-lock.json") },
            paths.GetLegacyGlobalLockPaths());
    }

    [Fact]
    public void GetLegacyGlobalLockPaths_IncludesXdgStateHome_When_Set()
    {
        // Arrange
        var env = new Dictionary<string, string?>(StringComparer.Ordinal) { ["XDG_STATE_HOME"] = "/custom/state" };
        var paths = Create(env);

        // Act & Assert: state-home location is preferred, with the ~/.agents path as a final fallback.
        Assert.Equal(
            new[]
            {
                Path.Combine("/custom/state", "skills", ".skill-lock.json"),
