        ...
        await interaction.StatusAsync("Removing skills...", async () =>
        {
            foreach (var skillName in selected)
            {
                try
                {
                    ...
                    bool lockEntryRemoved;
                    if (global)
                    {
                        lockEntryRemoved = await globalLock.RemoveEntryAsync(skillName, cancellationToken);
                    }
                    else
                    {
                        lockEntryRemoved = await projectLock.RemoveEntryAsync(skillName, cwd, cancellationToken);
                    }
