...
internal sealed class ConsoleInteractionService(IAnsiConsole? console = null) : IInteractionService
{
        ...
        where T : notnull
    {
        ...
        var preSelectedSet = preSelected is null
            ? null
            : new HashSet<T>(preSelected, EqualityComparer<T>.Default);
        ...
        {
            var index = i;
            prompt.AddChoices(index, item =>
            {
                if (preSelectedSet?.Contains(pairs[index].Value) == true)
                {
                    item.Select();
                }
            });
